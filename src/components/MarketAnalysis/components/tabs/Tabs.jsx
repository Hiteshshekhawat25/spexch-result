...
const Tabs = ({data, size, activeTab, setActiveTab, hasPadding, fancyTabs, highlightTab, casinoTab, inplayTab}) => {
  ...
  return (
    <div className={`flex overflow-y-auto max-lg:justify-start relative ${inplayTab ? '!gap-0 border border-[#3B5160] sm:inline-flex rounded overflow-hidden' : ''} ${highlightTab ? 'bg-gradient-green-hover max-lg:pt-2 px-2' : 'gap-2'} ${hasPadding ? 'pb-1' : ''}`}>
      {
        data?.length ? 
          data?.map((item) => (
