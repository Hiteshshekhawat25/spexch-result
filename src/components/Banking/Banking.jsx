...
const Banking = () => {
  ...
  const handleButtonClick = (status, index) => {
    setEditedData((prevState) => {
      ...
      // If "W" is clicked, highlight "Full"
      if (status === "W") {
        updatedData[index].highlightFull = true;
      } else {
        updatedData[index].highlightFull = false;
      }
  
      // If "Full" is clicked, auto-fill `depositwithdraw` with `totalBalance`
      if (status === "Full") {
        updatedData[index].depositwithdraw = filteredData[index]?.totalBalance || "";
        updatedData[index].highlightFull = true; // Keep "Full" highlighted
      }
