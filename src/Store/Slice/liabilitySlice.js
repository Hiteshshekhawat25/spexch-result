...
export const liabilityBook = createAsyncThunk('liability', async (data)=> {
  try {
    ...
    const response = await axios.get(`${BASE_URL}/user/get-pending-liability-list?page=${data.page}&limit=${data.limit}&sport=${data.sport}&matchId=${data?.matchId}&sessionId=${data?.sessionId}${data?.status == 'settled' ? `&isSettled=${data.status}` : ''}&type=${data?.type}${data.status !== 'settled' ?  `&deleteStatus=${data?.status}` : ''} `, {
      headers: {
